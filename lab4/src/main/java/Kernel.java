...
public class Kernel extends Thread
{
  ...
  private static int virtPageNum = 64;
  ...
  public void init( String commands , String config )  
  {
      ...
      try 
      {
...

        {
          ...
          { 
            ...
            {
              doFileLog = false;
              output = "src/main/resources/tracefile";
            }              
            else
            {
              doFileLog = true;
              doStdoutLog = false;
              output = "src/main/resources/" + tmp;
            }
          }
    ...
    boolean[] isPageFrameUsed = new boolean[( virtPageNum + 1 ) / 2];
    for (i = 0; i < virtPageNum; i++)
    {
      Page page = (Page) memVector.elementAt(i);
      if ( page.physical != -1 )
      {
        isPageFrameUsed[page.physical] = true;
        map_count++;
      }
    ...
    if ( map_count < ( virtPageNum + 1 ) / 2 )
    {
      ...
      for (i = 0; i < virtPageNum; i++)
      {
        ...
        {
          for (; lastPageFrame < isPageFrameUsed.length; lastPageFrame++) {
            if (!isPageFrameUsed[lastPageFrame]) {
              page.physical = lastPageFrame;
              map_count++;
              break;
            }
          }
  ...
  public void step()
  {
    ...
    getPage( Utils.pageNum( instruct.addr , virtPageNum , block ) );
    ...
    {
      Page page = ( Page ) memVector.elementAt( Utils.pageNum( instruct.addr , virtPageNum , block ) );
      ...
      if ( page.physical == -1 ) 
      {
        ...
        PageFault.replacePageNRU( memVector , virtPageNum , Utils.pageNum( instruct.addr , virtPageNum , block ) , controlPanel );
